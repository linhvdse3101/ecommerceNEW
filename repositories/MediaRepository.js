...
class MediaRespository {
    ...
    async getBannersBySlug(payload) {
        ...
        const endPoint = `banners?populate[0]=items&populate[1]=items.image&filters[slug][$eq]=${payload}`;
    ...
    async getPromotionsBySlug(payload) {
        const endPoint = `promotions?populate[0]=items&populate[1]=items.image&filters[slug][$eq]=${payload}`;
        const reponse = await Repository.get(`${baseUrl}/${endPoint}`)
            .then((response) => {
                if (response.data) {
                    return response.data.data[0].attributes.items;
